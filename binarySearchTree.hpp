...
namespace ft
{
	...
	template <class T, class Compare = std::less<T>, class Node = node<T>,
	...
	class BST
	{
		public:
			...
			{
				_last_node = _node_alloc.alloc(1);
			...
			{
				_remove_by_key(_last_node->parent, to_remove );
			}
		...
		private:
			...
			void	_replaceDoubleChildren( node_pointer& to_remove, node_pointer new_node)
			{
				...
				if (to_remove->left != _last_node && to_remove->right != new_node)
					to_remove->left->parent = new_node;
				if (to_remove->right != _last_node && to_remove->left != new_node)
			...
			void	_remove_by_key( node_pointer node, value_type to_remove)
			{
				if (to_remove.first < node->value.first)
				{
					_remove_by_key(node->left, to_remove);
					return ;
				}

				if (to_remove.first > node->value.first)
				{
					_remove_by_key(node->right, to_remove);
					return ;
				}
